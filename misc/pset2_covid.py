...
covid.country.replace({"United Kingdom": "U.K.",
                       "South Africa": "S. Africa"},
                      inplace=True)

# Filtering for data we're interesting in plotting
covid = covid.loc[(covid.type == "confirmed") &
                  (covid.country.isin(["U.K",
                                       "Argentina",
                                       "Italy",
                                       "Colombia",
                                       "Mexico",
...
covid_plot = sns.catplot(x="country", y="cases", data=covid, kind="box",
                         height=8)
...
title = "Confirmed COVID19 cases ({} -> {})".format(covid.date.min().date(),
                                                    covid.date.max().date())

# Labels.
covid_plot.ax.set_title(title, {"fontweight": "bold",
                                "fontsize": 16})
